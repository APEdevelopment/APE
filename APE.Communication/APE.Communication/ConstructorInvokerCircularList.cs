...
namespace APE.Communication
{
    internal class ConstructorInvokerCircularList
    {
        ...
        public void AddToList(string name, Int64 typeCodeKey, IntPtr datastoreTypeHandle, Fasterflect.ConstructorInvoker constructorInvoker)
        {
        ...
        public void GetFromList(string name, Int64 typeCodeKey, IntPtr datastoreTypeHandle, out Fasterflect.ConstructorInvoker constructorInvoker)
        {
            //Search for the item starting at the youngest item in the list
            for (int i = m_OldestItemInList - 1; i >= 0; i--)
            {
                if (m_Name[i] == name && m_TypeCodeKey[i] == typeCodeKey && m_DatastoreTypeHandle[i] == datastoreTypeHandle)
                {
